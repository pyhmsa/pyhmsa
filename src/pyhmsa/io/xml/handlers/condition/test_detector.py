...
from pyhmsa.io.xml.handlers.condition.detector import \
    (PulseHeightAnalyserXMLHandler, WindowXMLHandler,
     _DetectorXMLHandler, DetectorCameraXMLHandler,
...
class TestPulseHeightAnalyserXMLHandler(unittest.TestCase):

    def setUp(self):
        unittest.TestCase.setUp(self)

        self.h = PulseHeightAnalyserXMLHandler()

        self.obj = PulseHeightAnalyser(1750, 32, 0.5, 4.5, PHA_MODE_DIFFERENTIAL)

        source = StringIO(u'<PulseHeightAnalyser><Bias Unit="V" DataType="float">1700.</Bias><Gain DataType="float">16.</Gain><BaseLevel Unit="V" DataType="float">0.7</BaseLevel><Window Unit="V" DataType="float">9.3</Window><Mode>Differential</Mode></PulseHeightAnalyser>')
    ...
    def testfrom_xml(self):
        ...
        self.assertAlmostEqual(1700, obj.bias.value, 4)
        self.assertEqual('V', obj.bias.unit, 4)
        self.assertAlmostEqual(16, obj.gain, 4)
        self.assertAlmostEqual(0.7, obj.base_level.value, 4)
        self.assertEqual('V', obj.base_level.unit, 4)
        self.assertAlmostEqual(9.3, obj.window.value, 4)
        self.assertEqual('V', obj.window.unit, 4)
        self.assertEqual(PHA_MODE_DIFFERENTIAL, obj.mode)
    ...
    def testto_xml(self):
        ...
        self.assertEqual('PulseHeightAnalyser', element.tag)
        self.assertEqual('1750', element.find('Bias').text)
        self.assertEqual('32', element.find('Gain').text)
        self.assertEqual('0.5', element.find('BaseLevel').text)
        self.assertEqual('4.5', element.find('Window').text)
        self.assertEqual(PHA_MODE_DIFFERENTIAL, element.find('Mode').text)
...
class TestWindowXMLHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        source = StringIO(u'<Window><Layer Material="Al" Unit="\u00b5m" DataType="float">1.</Layer></Window>')
        self.element = etree.parse(source).getroot()
    ...
    def testfrom_xml(self):
        ...
        self.assertAlmostEqual(1.0, obj.layers[0].thickness.value, 4)
...
class Test_DetectorXMLHandler(unittest.TestCase):
    ...
    def testfrom_xml(self):
        obj = self.h.from_xml(self.element)
        self.assertEqual('ELS', obj.signal_type)
        ...
        self.assertAlmostEqual(45.0, obj.elevation.value, 4)
        self.assertEqual(u'\u00b0', obj.elevation.unit)
        self.assertAlmostEqual(0.0, obj.azimuth.value, 4)
        self.assertEqual(u'\u00b0', obj.azimuth.unit)
        self.assertAlmostEqual(50.0, obj.distance.value, 4)
        self.assertEqual('mm', obj.distance.unit)
        self.assertAlmostEqual(20.0, obj.area.value, 4)
        self.assertEqual('mm2', obj.area.unit)
        self.assertAlmostEqual(1.0, obj.solid_angle.value, 4)
        self.assertEqual('sr', obj.solid_angle.unit)
        self.assertAlmostEqual(3.4, obj.semi_angle.value, 4)
...
class TestDetectorCameraXMLHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        source = StringIO(u'<Detector Class="Camera"><UPixelCount DataType="uint32">512</UPixelCount><ExposureTime Unit="ms" DataType="float">200.</ExposureTime><Magnification DataType="float">4.5</Magnification><FocalLength Unit="mm" DataType="float">80.</FocalLength><VPixelCount DataType="uint32">400</VPixelCount></Detector>')
        self.element = etree.parse(source).getroot()
    ...
    def testfrom_xml(self):
        ...
        self.assertAlmostEqual(200.0, obj.exposure_time.value, 4)
        self.assertEqual('ms', obj.exposure_time.unit)
        self.assertAlmostEqual(4.5, obj.magnification, 4)
        self.assertAlmostEqual(80.0, obj.focal_length.value, 4)
...
class TestDetectorSpectrometerXMLHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        source = StringIO(u'<Detector Class="Spectrometer"><ChannelCount DataType="uint32">4096</ChannelCount><Calibration Class="Constant"><Quantity>Energy</Quantity><Unit>eV</Unit><Value DataType="float">-237.098251</Value></Calibration><CollectionMode>Parallel</CollectionMode></Detector>')
        self.element = etree.parse(source).getroot()
...
class TestDetectorSpectrometerCLXMLHandler(unittest.TestCase):
    ...
    def setUp(self):
        ...
        source = StringIO(u'<Detector Class="Spectrometer/CL"><ChannelCount DataType="uint32">4096</ChannelCount><Calibration Class="Constant"><Quantity>Energy</Quantity><Unit>eV</Unit><Value DataType="float">-237.098251</Value></Calibration><Grating-d Unit="mm-1" DataType="float">800</Grating-d></Detector>')
        self.element = etree.parse(source).getroot()
    ...
    def testfrom_xml(self):
        obj = self.h.from_xml(self.element)
        self.assertAlmostEqual(800.0, obj.grating_d.value, 4)
...
class TestDetectorSpectrometerWDSXMLHandler(unittest.TestCase):
    ...
    def testfrom_xml(self):
        ...
        self.assertAlmostEqual(8.742, obj.crystal_2d.value, 4)
        self.assertEqual(u'\u00c5', obj.crystal_2d.unit)
        self.assertAlmostEqual(140.0, obj.rowland_circle_diameter.value, 4)
        self.assertEqual('mm', obj.rowland_circle_diameter.unit)
        self.assertAlmostEqual(1700, obj.pulse_height_analyser.bias.value, 4)
        self.assertEqual('V', obj.pulse_height_analyser.bias.unit, 4)
        self.assertAlmostEqual(16, obj.pulse_height_analyser.gain, 4)
        self.assertAlmostEqual(0.7, obj.pulse_height_analyser.base_level.value, 4)
        self.assertEqual('V', obj.pulse_height_analyser.base_level.unit, 4)
        self.assertAlmostEqual(9.3, obj.pulse_height_analyser.window.value, 4)
        ...
        self.assertAlmostEqual(1.0, obj.window.layers[0].thickness.value, 4)
        self.assertEqual('\u00b5m', obj.window.layers[0].thickness.unit)
...
class TestDetectorSpectrometerXEDSXMLHandler(unittest.TestCase):
    ...
    def testfrom_xml(self):
        ...
        self.assertAlmostEqual(180000.0, obj.nominal_throughput.value, 4)
        self.assertEqual('counts', obj.nominal_throughput.unit)
        self.assertAlmostEqual(11.1, obj.time_constant.value, 4)
        self.assertEqual('\u00b5s', obj.time_constant.unit)
        self.assertAlmostEqual(2000, obj.strobe_rate.value, 4)
